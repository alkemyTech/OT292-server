...
const Admin = {
  ...
  const Testimonio = {
     name : "General",
     desctiption : "Categorias generales",
     image: "Testimonio.png"
  }
  const TestimonioUpdate = {
    name : "General actualizado",
    desctiption : "Categorias generales actualizado",
     ...
     before(async()=>{
         ...
         describe('create Testimonials',()=>{
              it('Should return 400 if name is not provided',async()=>{
                  ...
                  expect(res.body.errors).to.have.property('errors');
                ...
                it('Should return 400 if Content is not provided',async()=>{
                    ...
                    expect(res.body.errors).to.have.lengthOf(2);
                    expect(res.body.errors).to.deep.equal([
                      'body[name]: Content cannot be empty',
                      'body[name]: Content must be string',
                    ]);
                  })    
                it('Should return 400 if Image is not provided',async()=>{
                        const res = await chai
                          ...
                          .post('/')
                          .set('Authorization',`Bearer ${adminToken}`)
                          .send({name: 'nombre testimonio',content : 'contenido testimonio'});
                        expect(res.status).to.equal(400)
                        expect(res.body.status).to.equal(400)
                        expect(res.body.errors).to.have.property('errors');
                        expect(res.body.errors).to.be.an('array');
                        expect(res.body.errors).to.have.lengthOf(2);
                        expect(res.body.errors).to.deep.equal([
                          'body[name]: Image cannot be empty',
                          'body[name]: Image must be string',
                        ]);
                    })            
         ...
         describe('get Testimonials',()=>{
             ...
             it('get Testimonials 403 Forbidden',async()=>{
                    const res = await chai
                      .request(app)
                      .get('/')
                      .set('Authorization',`Bearer ${userToken}`);
         ...
         describe('put Testimonials',()=>{
            ...
            it('Should return 200 Update Testimonial Successful',async()=>{
                ...
                expect(res.status).to.equal(200);
                expect(res.body.status).to.equal(200);
                expect(res.body).to.have.property('id').that.equal(1);
                expect(res.body).to.have.property('message').to.be.an('object');
            });
          });
